...
frequency (instead of staying flat forever, as in the lossless-line
solvers), and check the roll-off against the exact analytic lossy
propagation constant.

## Scope: constant R, G (not skin-effect R ~ sqrt(f))

Real skin-effect loss has R growing like sqrt(f). That needs representing
a frequency-dependent impedance in the TIME domain, which usually means a
multi-pole (Foster network) approximation with an extra state variable
per pole, convolved into the update each step. That's a genuinely separate,
harder modeling technique from what's here.
...
this solver demonstrates and validates.

## Numerics: semi-implicit loss (why not just add R, G directly)

The reflection solver found that treating a boundary's loss term fully
explicitly gives it its own (tighter) stability limit. The same risk
applies here, along the WHOLE line, not just the boundary. The fix is the
standard one from
...
BECAUSE this treatment was used from the start (a case of an earlier
lesson from this project paying off directly).
...
needed like the S-parameter solver, because with a matched load there's no
reflection to separate out; everything that ever arrives at the far point
is the direct, attenuated, dispersed wave.
...
attenuation over the same distance, matching the other two solvers'
matched-line behavior).

## What went wrong

1. **Assumed constant R, G would give a boring, flat loss curve.**
   First instinct: "real skin effect needs sqrt(f); without it, constant
   R/G loss will just be flat and uninteresting." Wrong: the
...
2. **A short capture can't resolve the low-frequency plateau.** The
   first attempt used a ~20ns simulation (matching the other two solvers'
   timescales) and zero-padded the FFT to get a finer-looking frequency
   axis. That
